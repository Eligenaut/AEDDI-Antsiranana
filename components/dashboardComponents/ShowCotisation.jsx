...
export function ShowCotisation({ isOpen, onClose, cotisationId, isAdmin, cotisations }) {
  ...
  const memberData = !isAdmin ? (() => {
    ...
    return c ? { statut_paiement: c.statut_paiement, montant_restant: c.montant_restant } : undefined;
  })() : undefined;

  const statutLabel = STATUT_OPTIONS.find(s => s.value === cotisation?.statut)?.label ?? '';
            ...
            {loading ? (
              ...
              <div className="p-6 space-y-5">
                ...
                {memberData?.statut_paiement && (
                  <>
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">Mon état de paiement</label>
                      <div className="w-full px-3 py-2 border border-gray-200 rounded-lg text-gray-900 font-medium bg-gray-50 text-sm">
                    ...
                    {memberData.statut_paiement === 'reste' && memberData.montant_restant && (
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1">Montant restant (AR)</label>
                        <div className="relative">
                          <div className="w-full pl-10 pr-3 py-2 border border-amber-200 rounded-lg text-amber-800 font-medium bg-amber-50 text-sm">
                            {Number(memberData.montant_restant).toLocaleString('fr-FR')} AR
                          </div>
