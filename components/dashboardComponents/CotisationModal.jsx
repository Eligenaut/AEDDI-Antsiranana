...
export function CotisationModal({ isOpen, onClose, user, onUpdate }) {
  ...
  const fetchCotisations = async (memberId) => {
    ...
    setError(null);
    try {
      const response = await axios.get(`${url}cotisations/member/${memberId}`, {
  ...
  const handleDelete = async (cotisation) => {
    ...
    setError(null);
    try {
      await axios.delete(`${url}cotisations/${cotisation.cotisation.id}/member/${activeUser?.id}`, {
        headers: getAuthHeaders(),
      });
      if (onUpdate) {
        onUpdate();
