    ...
    <>
      <header className="bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-700 shadow-lg border-b border-purple-500">
        <div className="lg:hidden fixed top-0 left-0 right-0 z-40 bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-700 px-2 py-2 h-16 flex items-center justify-between">
          ...
          <div className="flex-1 text-center cursor-pointer" onClick={() => onSectionChange('accueil')} title="Aller au tableau de bord">
            <h2 className="text-xl font-extrabold tracking-wide text-white">AEDDI</h2>
          </div>
        ...
        <div className="lg:hidden px-4 py-2 border-t border-purple-500/30">
          <div className="flex items-center justify-between">
            <button className="p-3 text-white hover:bg-purple-500 rounded-lg transition-colors" onClick={() => onSectionChange('accueil')} title="Accueil">
        ...
        <div className="hidden lg:block px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <div className="hidden lg:flex items-center space-x-4 ml-4">
              ...
              <button className="p-2 text-white hover:text-purple-200 rounded-lg transition-colors flex items-center" title="Mon compte">
                <UserAvatar user={user} size="lg" />
                <span className="ml-2 hidden xl:inline">Mon compte</span>
              </button>
              <button onClick={handleLogout} className="p-2 text-white hover:text-red-200 hover:bg-red-500/20 rounded-lg transition-colors flex items-center" title="Se déconnecter">
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-700 shadow-t rounded-t-lg border-t border-purple-500/30 flex justify-around items-center py-2 lg:hidden">
        <button
          className={`flex flex-col items-center transition-colors ${currentSection === 'accueil' ? 'text-white font-bold' : 'text-white/80 hover:text-white focus:text-white'}`}
        ...
        <button
          className={`flex flex-col items-center transition-colors ${currentSection === 'membres' ? 'text-white font-bold' : 'text-white/80 hover:text-white focus:text-white'}`}
          onClick={() => onSectionChange('membres')}
        >
          <svg className="w-7 h-7 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m9-4V6a4 4 0 00-8 0v4m12 0a4 4 0 01-8 0m8 0v4a4 4 0 01-8 0v-4" />
          </svg>
          <span className="text-xs font-medium">Membres</span>
        </button>
        <button
          className={`flex flex-col items-center transition-colors ${currentSection === 'activites' ? 'text-white font-bold' : 'text-white/80 hover:text-white focus:text-white'}`}
          onClick={() => onSectionChange('activites')}
        >
          <svg className="w-7 h-7 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 21h8M12 17v4m-6-8h12a2 2 0 002-2V7a2 2 0 00-2-2H6a2 2 0 00-2 2v4a2 2 0 002 2z" />
          </svg>
          <span className="text-xs font-medium">Activités</span>
        </button>
        <button
          className={`flex flex-col items-center transition-colors ${currentSection === 'cotisations' ? 'text-white font-bold' : 'text-white/80 hover:text-white focus:text-white'}`}
